...
public class BoardController{
	...
	//Ranking id 2위인 사람을 전역변수에 넣는다//
	private String SecondRankingid;
	//Ranking id 3위인 사람을 전역변수에 넣는다//
	private String ThirdRankingid;
	
	private String Admin;
	private String uploadPath = "/Users/helloworld/eclipse-workspace/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/exhibition 2/resources/assets/img"; // folder내 파일 저장 경로
	private String uploadPath1 = "./resources/assets/img/"; // DB 저장용 경로
	private static final String FILE_SERVER_PATH = "/Users/helloworld/eclipse-workspace/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/exhibition 2/resources/assets/img";
	...
	public String RankingStyle(HttpServletRequest req, Model model) {
		...
		//2위 의  아이디와 , 조회수를 가져옴 (Sql Ranking 함수를 이용)
		BoardDto SecDto = dao.SecondRanking();
		//2위에 해당하는 유저의 아이디와 조회수를 view 전달 //
		model.addAttribute("SecDto", SecDto);
		
		
		//2위 한유저의 아이디를 셀렉트해서 담아줌 
		String SecId = dao.SecondRankingId();
		SecondRankingid = SecId;
		//2위한 유저의 총 게시물 개수를 구한다 
		String SecDao = dao.SecondRankingUserCount(SecondRankingid);
		System.out.println("Debuging-second1");
		System.out.println(SecondRankingid);
		System.out.println("Debuging-second2");
		model.addAttribute("SecDao", SecDao);
		System.out.println("Debuging-second3");
		
		System.out.println("Debuging-second4");
		//3위 의 아이디, 조회수를 가져옴(sql Ranking)
		BoardDto ThirDto = dao.ThirdRanking();
		model.addAttribute("ThirDto", ThirDto);
		//3위한 유저의 아이디를 셀렉트해서 담아줌 
		String ThirdId = dao.ThirdRankingId();
		System.out.println("Debuging-second5");
		ThirdRankingid = ThirdId;
		System.out.println("Debuging-second6");
		//3위한 유저의 총 게시물 개수를  구한다 .
		String ThirdDao = dao.ThirdRankingUserCount(ThirdRankingid);
		System.out.println(ThirdRankingid);
		model.addAttribute("ThirdDao", ThirdDao);
	...
	public String board_write_view(@RequestParam("uploadFile") MultipartFile file, ModelAndView mv,HttpServletRequest req, Model model) throws IOException {
		...
		String board_comment = req.getParameter("board_comment");
		BoardDao dao = sqlSession.getMapper(BoardDao.class);
		dao.write(boardtopic, userid, board_comment,file);
		return "redirect:http://localhost:8080/project/board?pageNum=1";
	}
	//테스트용 
