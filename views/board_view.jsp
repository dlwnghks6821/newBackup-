...
<p class="writer"><h3> 작성자 :<input type="text" id="writer"readonly value="${reply_view.userid }"></h3>
...
<h5>조회수:${reply_view.nClick}</h5>
  ...
  <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" style="height:400px" readonly>${reply_view.board_comment }</textarea>
    ...
    <button type="button" id="update" class="btn btn-primary"><a class="atag" href="http://localhost:8080/project/board_update?board_num=${reply_view.board_num }">글삭제하기/수정하기</a></button>
...
$(document)
	.on("click","#update",function(){
		if($("#hiddenUserid").val()!=$("#writer").val()){
			alert("권한이 없습니다.");
			return false;
		}
	})
